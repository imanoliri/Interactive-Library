...
from scripts.combat_templates import COMBAT_TEMPLATES
from scripts.book_template import INTERACTIVE_BOOK_TEMPLATE, CONTENTS_PAGE_TEMPLATE
...
def generate_contents_page(content_links):
    ...
    return CONTENTS_PAGE_TEMPLATE.replace("{buttons}", button_html)


def generate_static_html(chapters, tab_names, title):
    template = Template(INTERACTIVE_BOOK_TEMPLATE)
    return template.render(
        chapters=chapters, tab_names=tab_names, title=title.replace("_", " "),
        combat_templates=COMBAT_TEMPLATES
    )
